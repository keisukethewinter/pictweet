...
RSpec.describe 'ツイート削除', type: :system do
  ...
  context 'ツイートが削除できるとき' do
    it 'ツイートしたユーザーは自らが投稿したツイートの削除ができる' do
      # ツイート1を投稿したユーザーでログインする
      sign_in(@tweet1.user)
      # ツイート1に「削除」ボタンがある
      ...
      # 投稿を削除するとレコードの数が1減る
      expect{
        all(".more")[1].hover.find_link('削除', href: tweet_path(@tweet1)).click}.to change{Tweet.count}.by(-1)
      # 削除完了画面に遷移する
      expect(current_path).to eq tweet_path(@tweet1)
      # 「削除が完了しました」の文字がある
  ...
  context 'ツイート削除ができない時' do
    it 'ログインユーザーは自分以外が投稿したツイートの削除ができない' do
      # ツイート1を投稿したユーザーでログインする
      sign_in(@tweet1.user)
      # ツイート2に「削除」ボタンが無い
      expect(
...
RSpec.describe 'ツイート詳細', type: :system do
  ...
  it 'ログインしたユーザーはツイート詳細ページに遷移してコメント投稿欄が表示される' do
    ...
    expect(page).to have_selector ".content_post[style='background-image: url(#{@tweet.image});']" #× 画像が含まれるか見るコード
    expect(page).to have_content("#{@tweet.text}") #× 惜しい
    # コメント用のフォームが存在する
  ...
  it 'ログインしていない状態でツイート詳細ページに遷移できるもののコメント投稿欄が表示されない' do
    ...
    # ツイートに「詳細」ボタンがある
    expect(
      all(".more")[0].hover  #× [0]が抜けた
    ).to have_link '詳細', href: tweet_path(@tweet)
    # 詳細ページに遷移する
    visit tweet_path(@tweet) #@tweet1としない
    # 詳細ページにツイートの内容が含まれている
